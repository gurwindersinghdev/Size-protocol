...
library RiskLibrary {
    ...
    function collateralRatio(State storage state, address account) public view returns (uint256) {
        ...
        uint256 debtWad = Math.amountToWad(debt, state.data.underlyingBorrowToken.decimals()); //  amount * 10 ** (18 - decimals);
        uint256 price = state.oracle.priceFeed.getPrice();

        if (debt != 0) {
            return Math.mulDivDown(collateral, price, debtWad);  // c * p / d
        } else {
