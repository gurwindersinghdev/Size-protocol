        ...
        returns (uint256 liquidatorProfitCollateralToken)
    {
        ...
        if (assignedCollateral > debtInCollateralToken) {
            uint256 liquidatorReward = Math.min(
                ...
                Math.mulDivUp(debtPosition.futureValue, state.feeConfig.liquidationRewardPercent, PERCENT)       // x*y / d
            );
            ...
            uint256 collateralRemainderCap =
                Math.mulDivDown(debtInCollateralToken, state.riskConfig.crLiquidation, PERCENT); // x * y / d

            collateralRemainder = Math.min(collateralRemainder, collateralRemainderCap);

            protocolProfitCollateralToken = Math.mulDivDown(collateralRemainder, collateralProtocolPercent, PERCENT); // x * y / d
        } else {
